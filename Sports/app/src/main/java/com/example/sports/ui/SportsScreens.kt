...
fun SportsApp(
    ...
    Scaffold(
        topBar = {
            SportsAppBar(
                showUpButton = listType == SportsContentType.ListOnly,
                isShowingListPage = uiState.isShowingListPage,
                onBackButtonClick = { viewModel.navigateToListPage() },
            )
        }
    ) { innerPadding ->
        if (listType==SportsContentType.ListOnly) {
            ...
            SportListAndDetails(viewModel =viewModel,uiState = uiState, onBackPressed = { activity.finish() }, contentPadding = innerPadding)
        }
    }
}
...
fun SportsAppBar(
    ...
    TopAppBar(
        ...
        navigationIcon = if (!isShowingListPage && showUpButton) {
            {
...
private fun SportListAndDetails(
    ...
    Row {
        SportsList(
            contentPadding = PaddingValues(vertical = contentPadding.calculateTopPadding()),
        ...
        SportsDetail(selectedSport = uiState.currentSport, onBackPressed = onBackPressed, contentPadding = contentPadding,modifier = Modifier.weight(2f))
    }
}
