...
class RaceParticipantTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun raceParticipant_pausedAfterThreeDurationIntervals_ProgressUpdated() = runTest{
        launch { raceParticipant.run() }
        advanceTimeBy(raceParticipant.progressDelayMillis*3)
        runCurrent()
        assertEquals(3,raceParticipant.currentProgress)
    }
    ...
    @Test
    fun raceParticipant_Initialization_InitializedProgressValue() = runTest {
        launch { raceParticipant.run() }
